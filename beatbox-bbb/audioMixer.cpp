...
/**
 * audioMixer.cpp generates audio of the drum beats in real time and can set the volume
 * 
 */
...
void AudioMixer_init(void) {
	...
	playbackBuffer = (short int*)malloc(playbackBufferSize * sizeof(*playbackBuffer));
...
void AudioMixer_readWaveFileIntoMemory(char const *fileName, wavedata_t *pSound) {
	assert(pSound);
	...
	pSound->pData = (short int*)malloc(sizeInBytes);
	if (pSound->pData == 0) {
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	for(i = 0; i < MAX_SOUND_BITES; i++) {
		// thread safe access to soundBites[] 
	...
	if(i == MAX_SOUND_BITES) {
		printf("Error: soundBites[] queue full.\n");
	} 
}
...
static void fillPlaybackBuffer(short *playbackBuffer, int length)
{
	/*
	...
	unsigned long playbackBufferSizecopy = playbackBufferSize;

	memset(playbackBuffer, 0, playbackBufferSize * sizeof(*playbackBuffer));


	static short *playbackBuffercopy = playbackBuffer;
	//  (short int*)malloc(playbackBufferSize * sizeof(*playbackBuffer))
	...
	*/
	...
	// NOTE: pSound->numSamples = sizeInBytes / SAMPLE_SIZE;

	// Fill up the playbackBuffer array
	int buffFilled = 0;
	while (buffFilled < length) {
		signed long PCM_long = 0;
		pthread_mutex_lock(&audioMutex);
		// thread safe access to soundBites[] 
		for(int i = 0; i < MAX_SOUND_BITES; i++) {
			...
			if(pSound != NULL) {
				...
				short newPCM;
				// memcpy(&newPCM, pSound->pData + offset , sizeof(short) );


				// int samplesRead = fread(pSound->pData, SAMPLE_SIZE, pSound->numSamples, file);

				memcpy(&newPCM, pSound->pData + offset , SAMPLE_SIZE );
				

				PCM_long += newPCM;
				offset += SAMPLE_SIZE;

				// if end of sample data reached, free index
				if(offset >= soundBites[i].pSound->numSamples) {
					soundBites[i].pSound = NULL;
					assert(soundBites[i].pSound == NULL);
				} else {
		...
		pthread_mutex_unlock(&audioMutex);	
		// Clip PCM to short int (16)
		PCM_long = (PCM_long < SHRT_MIN) ? SHRT_MIN : PCM_long;
		short int PCM_short = (PCM_long > SHRT_MAX) ? SHRT_MAX : PCM_long;
		playbackBuffer[buffFilled] = PCM_short;
